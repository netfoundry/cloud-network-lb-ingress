...
asset=$(/usr/bin/gh api /repos/netfoundry/zfw/actions/artifacts --jq '( last ((.artifacts | sort_by(.created_at)).[] | select(.name | endswith("router-amd64-deb")).archive_download_url))')
